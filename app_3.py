...
def summarize_text(text, language=None):
    try:
...
def question_text(text, question, language=None):
    try:
        model = genai.GenerativeModel("gemini-1.5-flash")
...
# Streamlit app
def main():
    st.title("Multiple PDF Summarizer and Question Answering in Multilingual with AI")
    ...
    if uploaded_files:
        ...
        # Language selection
        language = st.text_input("Enter language for output (leave blank for English)")
        ...
        if st.button("Get Answers"):
            if question:
                for file_name, text in all_texts.items():
                    answer = question_text(text, question, language)
